...
export const generateImageForConcept = async (prompt: string): Promise<string> => {
  try {
    ...
    // Using imagen-3.0-generate-001 (more widely available)
    ...
    if (!base64ImageBytes) {
      console.error("No image bytes in response:", response);
      throw new Error("No image generated - check if Imagen API is enabled in your Google Cloud project");
    }
    ...
    if (error.status === 403 || error.status === 401) {
      throw new Error("Imagen API access denied. Please enable Imagen API in your Google Cloud Console and ensure billing is set up.");
    } else if (error.status === 404) {
      throw new Error("Imagen model not found. The model may not be available in your region.");
    }
